"""step2_world.launch.py - the plant, and only the plant.
...
Usage (after sourcing /opt/ros/jazzy/setup.bash):
  ros2 launch m5_ver2/step2/gazebo/step2_world.launch.py
...
for _path in (_WORLD, _MODEL, _IO_SCRIPT, _STO_SCRIPT):
    if not os.path.isfile(_path):
        raise FileNotFoundError(
            "step2_world.launch.py: no such file: {}".format(_path))
...
# m5_ver2/step2/ consumes linear_speed, fork_height or joint_states, and a
...
def generate_launch_description():
    ...
    #   and `gz sim` in step2.sh's PATTERNS nominates it.

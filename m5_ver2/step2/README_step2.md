# Step 2 — E-Stop chain end to end, plus three safety scanners
...
> tree. The last task of Step 2 rewrites this file and `PROOF.md` supplies the
> evidence.
...
| 3 | WSL | `./m5_ver2/step2/step2.sh start` — takes ~7 s. Do **not** source ROS first; the script does it. |
| 3a | Screen | Two windows appear: the **Gazebo window** with the warehouse and the forklift standing in the dock aisle, and the HMI joystick. `./m5_ver2/step2/step2.sh start --headless` skips the Gazebo one. |
| 4 | WSL | Read the four pid lines it prints. `WARNING: <name> exited during startup` sends you to that log in `m5_ver2/step2/logs/`. |
| 5 | Windows | `cd C:\Users\ozkan\projects\amr-agent` |
| 6 | Windows | `python m5_ver2\step2\windows\step2.py` — **64-bit Python** (pythonnet). It prints `streaming PLC state to <wsl-ip>:5100`. |
...
| 10 | WSL | `./m5_ver2/step2/step2.sh stop` |
...
### What `step2.py` accepts on stdin
...
### Two things `step2.sh` does that are easy to miss
...
- **`stop` validates the PID file before it signals anything.** Each recorded pid
  must still have `m5_ver2/step2` in its `/proc/<pid>/cmdline`, and each candidate
...
| `forklift_io` logs `waiting for source data: joint_states=False, odom=False` every 5 s, forever. | `joint_states` and odometry are deliberately not bridged — nothing in `m5_ver2/step2/` consumes them. The warning gates only two derived state scalars and the fork target seed, never the traction or steer command path. |
...
| `logs/plc_link.log` and `logs/cmd_gate.log` end in an `rclpy.executors.ExternalShutdownException` traceback. | That is what a clean SIGTERM looks like in these nodes — `step2.sh stop` sent it. It is the house pattern in `agv/`, and it appears *after* the node's normal startup line, not instead of it. |
...
|---|---|---|---|
| `windows/step2.py` | `PLC_INSTANCE` | `"PLC_2"` | error `-4` (`DoesNotExist`) = instance not running, or the name differs |
...
| `step2.sh` | `GZ_PARTITION` | `step2` | exported to every child; it is what scopes `stop`. Overridable from the environment. The GUI client inherits it, which is what makes it show *this* world rather than an empty scene. |
| | `ROS_DOMAIN_ID` | `92` | does **not** isolate Gazebo — gz transport is not DDS |
| | `GUI` | `true` | `start` opens the Gazebo window; `start --headless` sets it false. `gazebo/step2_world.launch.py` declares `gui` with the opposite default (`false`), so a bare `ros2 launch` is unchanged. |
...
| Vehicle stops after the PLC link dies (`step2.py` killed) | detected in ≤ 350 ms; budget < 0.45 s end to end |
...
[ ] step2.sh start brings up Gazebo (warehouse + forklift) and HMI
...
[ ] step2.sh stop kills everything, no orphan processes
```
...
would be claiming a run that never happened. This checklist is Step 2's to

...
# sets LD_LIBRARY_PATH to the lib dir printed at the end; step6.sh does
...
# MAINTENANCE OBLIGATION: step6.sh spells the same path in BROKER_LIB,
...
#   report as a failed install (the same trap as step6.sh's port guard).

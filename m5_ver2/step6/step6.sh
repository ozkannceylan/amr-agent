#!/usr/bin/env bash
# step6.sh - bring the Step 5 vehicle side up and down.
#   start [--headless] | stop
#
...
STEP5="$REPO/m5_ver2/step6"
PIDFILE="$STEP5/.step6_pids"
LOGDIR="$STEP5/logs"
DEPLOY="$STEP5/deploy"
...
home() {
    ...
    # step6_world.launch.py is the one home the spawn pose has; a literal
    ...
    # live simulator.
    local pid p launch="$STEP5/gazebo/step6_world.launch.py"
    if [ -f "$PIDFILE" ]; then
...
deploy() {
    ...
    # deploy/m5_ver2/step6/ipc/cmd_gate.py finds config.yaml at
    ...
    cp -r "$STEP5/ipc" "$DEPLOY/m5_ver2/step6/ipc"
...
stale_check() {
    ...
    while read -r h p; do
        case "$p" in
            ./m5_ver2/step6/ipc/*) s="$STEP5/ipc/${p#./m5_ver2/step6/ipc/}" ;;
            ./agv/*)               s="$REPO/${p#./}" ;;
            *)                     continue ;;
        esac
    ...
    man_n=$(grep -c 'm5_ver2/step6/ipc' "$DEPLOY/MANIFEST")
    [ "$src_n" != "$man_n" ] && stale=1
    if [ "$stale" = 1 ]; then
        echo "  ================================================="
...
start() {
    ...
    # binds it; a step4 or step6 stack already up owns it, and the second
    ...
    # Unchecked, an unwritable log dir fails all nine redirections, and start
    ...
    spawn world  ros2 launch "$STEP5/gazebo/step6_world.launch.py" "gui:=$GUI"
    sleep 5
    ...
    echo "  python m5_ver2\\step6\\windows\\step6.py"
    echo "logs: $LOGDIR"
}
stop() {
    local pid p

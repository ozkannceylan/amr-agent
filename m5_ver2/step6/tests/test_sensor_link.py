...
def test_a_report_without_right_or_left_sends_nothing():
    ...
    # nothing lets step6.py's one staleness rule trip ALL six inputs
...
def test_a_non_boolean_verdict_sends_nothing():
    # Sending nothing is safe: step6.py's own timeout then trips within
...
def test_an_absent_encoder_channel_sends_nothing():
    ...
    # whole datagram is withheld and step6.py's timeout trips instead.
    assert sensor_link.payload(
        _fields(True, True),

"""step6.control_loop itself, driven against the virtual F-PLC.
...
def _pulse_ack(state):
    ...
    the future.
    """
    while time.monotonic() < state["ack_until"] + 2 * step6.CYCLE_S:
        time.sleep(0.01)
    state["ack_until"] = time.monotonic() + step6.ACK_PULSE_S
...
def rig():
    ...
    try:
        ...
        listener.close()
        pytest.skip(
            "UDP {} is already bound ({}) - a live step6.py or plc_link is "
            "holding the port this rig needs".format(step6.UDP_PORT, exc))

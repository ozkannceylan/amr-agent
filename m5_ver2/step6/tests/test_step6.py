"""step6.py's pure functions. Nothing here opens the PLCSIM API."""
import json
...
def test_status_payload_has_exactly_the_five_wire_keys():
    raw = step6.status_payload(True, False, 2, 1500, 12.5)
...
def test_status_payload_emits_real_json_booleans_not_strings():
    msg = json.loads(step6.status_payload(False, True, 3, 300, 0.0).decode())
...
def test_parse_sensor_accepts_the_nine_key_packet():
    msg = step6.parse_sensor(_sensor_msg())
...
def test_parse_sensor_rejects_a_packet_missing_a_side_verdict():
    ...
    assert step6.parse_sensor(json.dumps(old).encode()) is None


def test_parse_sensor_rejects_a_non_boolean_side_verdict():
    assert step6.parse_sensor(_sensor_msg(pf_left=1)) is None
    assert step6.parse_sensor(_sensor_msg(wf_right="clear")) is None


def test_resolve_udp_target_honours_an_explicit_string():
    assert step6.resolve_udp_target("10.0.0.5") == "10.0.0.5"
...
def test_resolve_udp_target_takes_the_first_token_of_the_wsl_reply():
    ...
    assert step6._first_token("172.19.180.72 172.17.0.1 \n") == "172.19.180.72"

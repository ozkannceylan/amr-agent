"""step5_world.launch.py - the plant, and only the plant.
...
Usage (after sourcing /opt/ros/jazzy/setup.bash):
  ros2 launch m5_ver2/step5/gazebo/step5_world.launch.py
  ros2 launch m5_ver2/step5/gazebo/step5_world.launch.py gui:=true
"""
...
for _path in (_WORLD, _MODEL, _IO_SCRIPT, _STO_SCRIPT):
    if not os.path.isfile(_path):
        raise FileNotFoundError(
            "step5_world.launch.py: no such file: {}".format(_path))
...
def generate_launch_description():
    ...
    #   and `gz sim` in step5.sh's PATTERNS nominates it.
    #

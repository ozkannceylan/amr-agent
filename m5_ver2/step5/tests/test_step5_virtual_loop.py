"""step5.control_loop itself, driven against the virtual F-PLC.

WHAT LEVEL OF EVIDENCE THIS IS, EXACTLY
  test_virtual_fplc.py proves the MODEL. These tests prove the LOOP that
  drives it: the real `step5.control_loop` - the same function the Windows
...
def _pulse_ack(state):
    ...
    the future.
    """
    while time.monotonic() < state["ack_until"] + 2 * step5.CYCLE_S:
        time.sleep(0.01)
    state["ack_until"] = time.monotonic() + step5.ACK_PULSE_S
...
def rig():
    ...
    try:
        listener.bind(("127.0.0.1", step5.UDP_PORT))
    except OSError as exc:
        listener.close()
        pytest.skip(
            "UDP {} is already bound ({}) - a live step5.py or plc_link is "
            "holding the port this rig needs".format(step5.UDP_PORT, exc))

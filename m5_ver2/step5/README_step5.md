# Step 5 — E-Stop chain end to end, plus three safety scanners

> **This page is still Step 4's, carried by the copy.** The last task of Step 5
> rewrites it; PROOF.md supplies the evidence.
...
| 3 | WSL | `./m5_ver2/step5/step5.sh start` — takes ~7 s. Do **not** source ROS first; the script does it. |
| 3a | Screen | Two windows appear: the **Gazebo window** with the warehouse and the forklift standing in the dock aisle, and the HMI joystick. `./m5_ver2/step5/step5.sh start --headless` skips the Gazebo one. |
| 4 | WSL | Read the four pid lines it prints. `WARNING: <name> exited during startup` sends you to that log in `m5_ver2/step5/logs/`. |
| 5 | Windows | `cd C:\Users\ozkan\projects\amr-agent` |
| 6 | Windows | `python m5_ver2\step5\windows\step5.py` — **64-bit Python** (pythonnet). It prints `streaming PLC state to <wsl-ip>:5100`. |
...
| 10 | WSL | `./m5_ver2/step5/step5.sh stop` |
...
### What `step5.py` accepts on stdin
...
### Two things `step5.sh` does that are easy to miss
...
- **`stop` validates the PID file before it signals anything.** Each recorded pid
  must still have `m5_ver2/step5` in its `/proc/<pid>/cmdline`, and each candidate
...
| `Motor` is OFF at a fresh start with nothing tripped, and one `a` is required before anything moves. | `ACK_NEC = true` in the ESTOP1 blocks: one `Acknowledge` rising edge is required after PLC startup before `Motor` can ever be True. You also need it after **every** `step5.py` run, because `q` and every exception write `E-Stop` and all six scanner inputs (`PF_OSSD`, `WF_Clear` and their `_right`/`_left` counterparts, since 2026-08-12) False on the way out — that trip latches too. |
...
| `forklift_io` logs `waiting for source data: joint_states=False, odom=False` every 5 s, forever. | `joint_states` and odometry are deliberately not bridged — nothing in `m5_ver2/step5/` consumes them. The warning gates only two derived state scalars and the fork target seed, never the traction or steer command path. |
...
| `logs/plc_link.log` and `logs/cmd_gate.log` end in an `rclpy.executors.ExternalShutdownException` traceback. | That is what a clean SIGTERM looks like in these nodes — `step5.sh stop` sent it. It is the house pattern in `agv/`, and it appears *after* the node's normal startup line, not instead of it. |
...
| `ipc/cmd_gate.py` | `ZERO_HZ` | `10.0` | load-bearing on the 0.45 s budget below — do not lower it |
...
| Vehicle stops after the PLC link dies (`step5.py` killed) | detected in ≤ 350 ms; budget < 0.45 s end to end |
...
[ ] step5.sh start brings up Gazebo (warehouse + forklift) and HMI
...
[ ] step5.sh stop kills everything, no orphan processes
```
...
would be claiming a run that never happened. This checklist is Step 5's to

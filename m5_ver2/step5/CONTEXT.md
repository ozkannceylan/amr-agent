...
Carried from Step 4 — rewritten by Step 5's final task.
...
top and what a Step 5 implementer must not break.
...
```
E-Stop button      step5.py terminal: es0 / es1
...
ceiling. `step5.py` injects the faults, because a broken encoder is a field
fault and the PLCSIM API is the wiring — the vehicle sends what the shaft
did and never lies.

## What will stop a Step 5 vehicle unexpectedly
...
`sensor_link.payload()` writes it, `step5.py parse_sensor()` validates it.
They are a pair. Changing one without the other is silent.

## Terminal commands on `step5.py`
...
`GZ_PARTITION=step5`, `ROS_DOMAIN_ID=95`. The next step takes `step6` / `96`, or
...
that survive into Step 5's tree:
...
- `step5.sh`'s startup name list is positional and must be hand-synced with
  the spawn order; `stack.sh:191-203` teaches the per-component token fix.
- No committed test covers `step5.py`'s fail-direction path.

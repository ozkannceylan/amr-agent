#!/usr/bin/env bash
# step3.sh - bring the Step 3 vehicle side up and down.
#   start [--headless] | stop
#
# start opens the Gazebo GUI client, because this script is the HUMAN entry
# point and the point of Step 3 is watching the truck stop. The launch file's
...
#
# It does NOT touch PLCSIM Advanced or step3.py. Those are the owner's, on
...
STEP3="$REPO/m5_ver2/step3"
PIDFILE="$STEP3/.step3_pids"
LOGDIR="$STEP3/logs"
ROS_SETUP="/opt/ros/jazzy/setup.bash"

export GZ_PARTITION="${GZ_PARTITION:-step3}"
...
PATTERNS=("gz sim" "step3_world.launch.py" "parameter_bridge" \
          "sto_contactor.py" "forklift_io.py" "plc_link.py" "cmd_gate.py" \
          "hmi_node.py" "field_eval.py" "sensor_link.py" "encoder_link.py")
...
#   a command line BYTE-IDENTICAL to step3_world.launch.py's - same absolute
...
#   owner started with GZ_PARTITION=step3 is by that act IN this graph.
...
#   recorded command lines contain m5_ver2/step3 and no foreign one does, so
#   that token is the identity test. It is deliberately the literal and not
#   "$STEP3": if REPO ever resolves differently between the start and the
...
recorded() {
    grep -qaF "m5_ver2/step3" "/proc/$1/cmdline" 2>/dev/null
}
...
sweep() {  # sweep <signal>
    ...
    for pat in "${PATTERNS[@]}"; do
        while read -r pid cmd; do
            ...
            case "$cmd" in *step3.sh*|*demo.sh*|*stack.sh*) continue ;; esac
            ours "$pid" || continue
...
start() {
    ...
    echo "starting the Step 3 vehicle side (partition $GZ_PARTITION, domain $ROS_DOMAIN_ID, gui $GUI)"
    spawn world  ros2 launch "$STEP3/gazebo/step3_world.launch.py" "gui:=$GUI"
    sleep 5
    spawn plc_link python3 "$STEP3/ros2/plc_link.py"
    spawn cmd_gate python3 "$STEP3/ros2/cmd_gate.py"
    # field_eval BEFORE sensor_link, so the link never sends a verdict from
    # a device that has not been evaluated yet.
    spawn field_eval   python3 "$STEP3/ros2/field_eval.py"
    spawn encoder_link python3 "$STEP3/ros2/encoder_link.py"
    spawn sensor_link python3 "$STEP3/ros2/sensor_link.py"
    spawn hmi         python3 "$STEP3/ros2/hmi_node.py"
    ...
    sleep 1
    local i=0 names=(world plc_link cmd_gate field_eval encoder_link sensor_link hmi) bad=0
    while read -r pid; do
        recorded "$pid" || { bad=1
    ...
    echo "  python m5_ver2\\step3\\windows\\step3.py"
    echo "logs: $LOGDIR"
}
stop() {
    local pid p

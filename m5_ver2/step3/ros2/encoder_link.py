...
THIS NODE NEVER LIES ABOUT WHAT IT MEASURED
  The fault modes live in step3.py, on the Windows side, because a broken
...
Usage (after sourcing /opt/ros/jazzy/setup.bash):
  python3 m5_ver2/step3/ros2/encoder_link.py
"""
...
CHANNEL_TOPIC = "/forklift/gz/drive_speed/read_{}"
CHANNELS = ("a", "b")
PUBLISH_HZ = 20.0
...
def load_wheel_radius(path=CONFIG_YAML):
    with open(path, "r", encoding="utf-8") as handle:
        return float(yaml.safe_load(handle)["model"]["wheel_radius_m"])
...
class EncoderLink(Node):
    ...
    def __init__(self):
        ...
        self.wheel_radius = load_wheel_radius()
        self.mm_s = {c: None for c in CHANNELS}
        self.last_rx = {c: None for c in CHANNELS}
        self.pub = self.create_publisher(String, ENCODERS_TOPIC, 10)
        for c in CHANNELS:
            self.create_subscription(
                JointState, CHANNEL_TOPIC.format(c),

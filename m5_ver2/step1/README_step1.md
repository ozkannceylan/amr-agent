...
| 3a | Screen | Two windows appear: the **Gazebo window** with the warehouse and the forklift standing in the dock aisle, and the HMI joystick. `./m5_ver2/step1/step1.sh start --headless` skips the Gazebo one. |
...
| `step1.sh` | `GZ_PARTITION` | `step1` | exported to every child; it is what scopes `stop`. Overridable from the environment. The GUI client inherits it, which is what makes it show *this* world rather than an empty scene. |
| | `ROS_DOMAIN_ID` | `91` | does **not** isolate Gazebo — gz transport is not DDS |
| | `GUI` | `true` | `start` opens the Gazebo window; `start --headless` sets it false. `gazebo/step1_world.launch.py` declares `gui` with the opposite default (`false`), so a bare `ros2 launch` is unchanged. |
...
[x] step1.sh start brings up Gazebo (warehouse + forklift) and HMI
...
Row 2 was ticked on 2026-08-12 from the screenshots in `assets/m5-step1-gui/`:
the window open on the warehouse, the forklift at the spawn pose beside the
column at (-4.60, -7.00), and the same camera after it was driven 12.5 m east
down the dock aisle. Everything below it is `PROOF.md`'s to tick, because it
needs `PLC_2`.

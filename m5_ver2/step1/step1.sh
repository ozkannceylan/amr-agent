...
# stack's publishers on this one's topics. They also decide what stop may
# kill - see ours(). Usage: step1.sh start | stop
set -uo pipefail
...
#   vehicle.launch.py:738-754 starts sto_contactor.py and forklift_io.py
#   with a command line BYTE-IDENTICAL to step1_world.launch.py's - same
#   absolute script path, same --config - and both stacks run gz sim on the
#   same warehouse.sdf. No substring of a command line tells this stack's
#   copy from the owner's, so `pkill -f forklift_io.py` would kill a live
#   M5 demo. What separates them is the isolation pinned above, which is
#   also the definition of "this graph": GZ_PARTITION, inherited by every
#   child through ros2 launch. An unreadable environ is left alone - the
#   safe direction. Accepted exposure: what the owner started with
#   GZ_PARTITION=step1 is by that act IN this graph, so it is swept.
#   demo.sh:1046 and stack.sh:46-50 scope sweeps the same way; the old
#   stack's partition is m5demo (demo.sh:121), not step1.
ours() {
...
start() {
    ...
    if [ -f "$PIDFILE" ]; then
        ...
        # Every pid gone: a crashed run left the file. The return above is
        # what keeps start from ever writing to a LIVE stack.
        rm -f "$PIDFILE"
    fi
...
stop() {
    ...
    #   controllers holding the last setpoint forever: after its publisher
    #   stopped, the truck ran 14.8 m on a standing command. Killing this
    #   stack therefore cannot slow a moving vehicle - not sto_contactor,
    #   whose latch is moot once nothing publishes through it, nor cmd_gate,
    #   whose zeros never arrive; doing either first would only leave a
    #   moving truck being integrated for the extra seconds the teardown
    #   takes. gz sim is where the motion lives, so PATTERNS puts it first:
    #   ending the simulation is the only stop this script owns. The brake
    #   is the e-stop, and it stays the e-stop.
    sweep TERM

...
| `tools/pallet_cycle.py` | **F5 Task 3 dry full cycle.** `describe` / `run [--repeat 3]`. Nav2 transit (`spine_north`) and empty stage (`stage_s5`). Pickup dock via `dock_bench.py --from-staging`. Attach/lift/lower/detach via `pallet_bench.py`. Undock, laden carry, laden return-stage and return-dock are cmd_vel bursts (not `UndockRobot` 905, not Nav2 through the payload). |

...
└── tools/
    ...
    └── film_run.py      THE FILM. `record` places the three film
                          ...
                          stamped into timeline.json, and every
                          RECOVERY INTERVENTION the cycle prints
                          stamped beside them by name. The cycle is
                          ...
                          nothing. The cut REFUSES a timeline holding
                          a recovery intervention - by name, before a
                          single recording is read and with no
                          override flag, because the film of a cycle
                          that needed a recovery is not the film of
                          autonomy (AMR-DEC-004) - a cycle that did not
...
| `m5v3.sh start --localize --nav --dock` | **F5 TASK 1-3: DETECTOR, S5 MARKER, PALLET AND THE DOCKING SERVER.** Spawns `tag36h11_0` via `furniture.py place` and `pallet_s5` via `pallet_place.py` (constraint 21), `pallet_cam_link` → `pallet_cam_optical` (REP-103), vendored `apriltag_node`, `detected_dock.py` (TF → `detected_dock_pose`) and `opennav_docking` `SimpleNonChargingDock`. **cmd_vel remapped onto `topics.cmd_vel` (constraint 22).** Refused without `--nav`. `dock=on@<apriltag.yaml md5>` `docking=on@<docking.yaml md5>`. Score detections: `tag_bench.py record`. Score the dock: `dock_bench.py record`. Pallet: `pallet_bench.py attach`. |
| `tools/furniture.py` | **F5 station furniture.** `describe` / `place` / `remove`. Constraint 21: gz create of the tag SDF, never a world edit. |
| `tools/pallet_place.py` | **F5 Task 3 pallet spawn.** `describe` / `place` / `remove`. Constraint 21: gz create of `pallet_s5.sdf`, never a world edit. |
| `tools/pallet_bench.py` | **F5 Task 3 attach/lift/detach.** `describe` / `status` / `attach` / `lift` / `lower` / `detach`. `attach` publishes only after `pallet_core.attach_ok` on live gz poses (constraint 23). |
| `tools/pallet_cycle.py` | **F5 Task 3 dry full cycle.** `describe` / `run [--repeat 3]`. Nav2 transit **chosen by origin** (G5 Task 9): cycle 1 leaves the spawn on `spine_north` (MPPI), every cycle after it leaves the S5 bay on `spine_north_from_bay` (RPP) — one target pose, two rows, because the two geometries measured opposite ways. Empty stage is `stage_s5`. Pickup dock via `dock_bench.py --from-staging`. Attach/lift/lower/detach via `pallet_bench.py`. Undock, laden carry, laden return-stage and return-dock are cmd_vel bursts (not `UndockRobot` 905, not Nav2 through the payload). |
| `tools/tag_bench.py` | **AprilTag pose vs the marker.** `describe` / `record` / `analyse`. `analyse` needs no ROS. Refuses a stack whose `dock=` line is not `on@…`. |
| `tools/dock_bench.py` | **F5 TASK 2's DOCK BENCH.** `describe` / `stage` / `record` [`--from-staging`] / `undock` / `analyse`. Cancels NavigateToPose, waits for `cmd_vel` quiet, then `DockRobot` (constraint 22). `stage` is gz `set_pose` + `/initialpose` at T1's heading-aligned staging (Nav2 does not deliver that heading). Session `authority=dock`. `analyse` needs no ROS. Refuses `nav=off` or `docking` not `on@…`. |
| `tools/install_apriltag.sh` | Vendors `apriltag_ros` into `$HOME` without sudo. Idempotent. `start --dock` refuses if it has not been run. |
...
| `tools/film_run.py record` | **THE F5 FILM, RECORDED.** Places the follow camera (7 m, moved to the truck through `set_pose` every 0.5 s), the fixed 45° dock camera over the S5 bay and the wide 22 m camera into the RUNNING world through `/create` — the world file is never edited — starts one `film_record.py` per camera plus the truck's own pallet camera, refuses by name any camera that stays silent for `film.camera_warmup_s`, holds twice `film.lead_s` of wall time so the establishing shot exists before the first leg does, and then runs **`pallet_cycle.py run` UNDER OBSERVATION, unmodified**: its stdout is read line by line and every `leg c1-<name>` is stamped with the wall clock into `timeline.json`. Every **recovery intervention** the cycle prints — the `nav2 miss recovered via staging` line it emits when a Nav2 miss is answered by teleporting the truck to staging — is stamped into the same timeline by name beside the legs, and a session that holds one is reported and exits 1 rather than offering a cut: the film of a cycle that needed a recovery is not the film of autonomy (AMR-DEC-004). When the cycle is `done` the session directory under `logs/film/` holds four mp4s, the three clock sidecars each recorder leaves beside its own (`.t0`, `.t1`, `.n`) and the timeline. Needs the full `--localize amcl --nav --dock` stack up and ROS sourced. |
| `tools/film_run.py cut [--session <dir>]` | **THE FILM, CUT.** Turns the latest recorded timeline plus its recordings into ONE film — `logs/film/<session>/m5v3-film.mp4`: the wide establishing shot, every cycle leg on the camera `film.shots` names (follow for the driving legs, dock for the pallet legs — a lift is a z-motion and only reads in profile), the vehicle camera inset over the approach legs, and a hold on the last leg. Every bound is a WALL time and every recording runs on the SIM clock, so each is mapped into its own file's seconds by the rate its `.t0`/`.t1`/`.n` sidecars measure — on a rig at 0.75 × wall a 349 s cycle is a 263 s film, and unmapped the last legs would fall off the end of the footage. The lead is planned from what EXISTS — its start clamps to the wide recording's own first frame and the printed `lead` line says how much of `film.lead_s` survived, because a short establishing shot is a degraded shot and not a lie. **Refuses** a timeline holding a recovery intervention — by name, quoting the line and where in the cycle it fell, before a single recording is read and with no override flag (AMR-DEC-004; a timeline with no `interventions` key at all was recorded before this check existed and cuts exactly as it did) — a cycle that did not finish, a timeline whose legs are not the shot table's, a follow or dock recording missing (or unreadable on) its sidecars, and a cycle segment needing footage more than `film.eof_tolerance_s` past the end of — or before the start of — its own recording — ffmpeg clamps such a trim at end-of-file, or at 0, silently; a missing wide or vehicle recording is still cut around with a printed line. Needs no ROS — ffmpeg and the session's own files only. |
...
| `/forklift/gz/cam/image` | gz → ROS | 15 Hz | **image bridge** (F5 colour / AprilTag) |
...
lidar), both point clouds, and
`/forklift/gz/drive_speed/read_b`. The colour image IS bridged since F5
(AprilTag). The first three still have no ROS consumer — F2 Task 1's EKF
fuses the IMU and the wheel odometry and

...
| `m5v3.sh start --localize --nav --dock` | **F5 TASK 1+2: DETECTOR, S5 MARKER AND THE DOCKING SERVER.** Spawns `tag36h11_0` via `furniture.py place` (constraint 21), `pallet_cam_link` → `pallet_cam_optical` (REP-103), vendored `apriltag_node`, `detected_dock.py` (TF → `detected_dock_pose`) and `opennav_docking` `SimpleNonChargingDock`. **cmd_vel remapped onto `topics.cmd_vel` (constraint 22).** Refused without `--nav`. `dock=on@<apriltag.yaml md5>` `docking=on@<docking.yaml md5>`. Score detections: `tag_bench.py record`. Score the dock: `dock_bench.py record`. |

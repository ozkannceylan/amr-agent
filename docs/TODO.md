...
- Hermes: m4-00 survey done and redacted 2026-07-28 (docs/reports/m4-00-hermes-survey.md — architectural content only; the infrastructure inventory was removed for publication per pub-01). Parked at M11 by ADR 0007. Headline decisions before it can be briefed: (1) rule invariant 8 — the assistant holds the OPC UA client, or the tailnet carries intent only and a cell-side executor writes; note its framework cannot reach private or CGNAT addresses with its web tool, so any assistant-side path is command-line shaped regardless; (2) rule what "Telegram-triggered" means, since the assistant also runs scheduled jobs and subagents and a command need not have a human in the loop; (3) the Command node group must be designed — no existing node may legitimately take a command; (4) topology has no operator/assistant box, placement needs an ADR on the ADR 0005 precedent; (5) integration code does NOT belong in the assistant's own repository, which it can push to as its deployment path. Caveat for all future briefs: no checkout is authoritative about what the deployment currently runs.
...
## verifier
- m3-37 M3 gate verification (issued) — done when each of the four exit items carries a met/not-met ruling against the roadmap's own text, the two knowingly-open belt-plausibility items and 4.9b's partial status are ruled on by name, and the report ends with one verdict plus the sentence roadmap.md should carry if it closes.

## interface (fold together, from m3-35 and m3-36)
- bridge-design.md §8.1 has no restart-detection row although the shipped code's log line cites it; §2's cycle description predates the once-per-cycle heartbeat read-back. Also SPEC §12 item 7 is now satisfied by behaviour (plc/ to close).

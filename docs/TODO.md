...
- Hermes: m4-00 survey done 2026-07-28 (docs/reports/m4-00-hermes-survey.md, ten decisions). The named checkout hermes-assistant does not exist; the survey covered rookie-assistant, which matches the description but is stale (HEAD 2026-05-04 while the deploy hard-resets the VPS to origin/main). Top decisions before M4 is briefed: (1) confirm the checkout is Hermes and refresh it; (2) rule invariant 8 — Hermes holds the OPC UA client over the tailnet (engineering-access reading) or the tailnet carries intent only and a cell-side executor writes (cell-traffic reading); (3) rule whether ADR 0004's "same server" premise is superseded; (4) choose the M4 command node group — no existing node may legitimately take a command (Input/ is bridge-only, Output/ is the actuator, Status/ is read-only); (5) Hermes has no place in the §3 topology, so placement needs an ADR on the ADR 0005 precedent.

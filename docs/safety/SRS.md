...
| Acceptance test | **AT-02** (Gazebo, M5 — restated 2026-08-06, m5-43: the M5 **instrument** is narrowed here; the SF-02 claim in the rows above is not). **What M5 has:** the vehicle model carries no onboard e-stop channel; the only e-stop channel built is SF-01's cell channel, and B4 forbids driving any vehicle stop from it, so no M5 run may present a cell e-stop actuation as this function. **M5 form — the stop-and-recovery discipline, exercised on the one vehicle-stopping demand M5 has** (the F-latched protective demand of AT-03 (a), acting through the standard program's envelope and the vehicle's enforcement gate): (a) while the vehicle drives a Nav2 path at nominal speed, latch the demand → the gate cuts the drive command **below Nav2** (bypassed, not asked) and the vehicle decelerates to standstill — the demand read in the **consumer's view** (`InstF_Forklift_Safety` instance data, separate process), witnessed on the read-only `Forklift/Safety/` mirror, standstill read on the vehicle's odometry topic; (b) Nav2 goals and command messages published during the latched stop produce **no motion and no non-zero command downstream of the gate**; (c) clearing the stimulus alone restores nothing; the SF-08 monitored reset (cell instance — the vehicle instance is deferred with the channel) clears the latch, and motion resumes only on a **fresh** navigation goal. **A failing run looks like:** any Nav2 command during the latched stop producing motion or a non-zero command below the gate; motion returning on reset alone without a fresh goal; or motion returning on stimulus-clearance without a reset. **Deferred, 2026-08-06 → M6:** the e-stop stimulus itself — a two-channel onboard e-stop stand-in and its stop-category-0 / STO semantics — the vehicle SF-08 reset instance, and `safetyState.eStop` = `MANUAL`, which is first producible at M6, where the VDA 5050 client exists and the onboard chain gives it something true to report; it has no M5 stand-in because the function it reports has no M5 instance. These land with the onboard safety model AT-09 already requires. Pass: (a)–(c) observed in one run; the inhibit demonstrably acts below the navigation stack. |
...
| Acceptance test | **AT-03** (Gazebo, M5 — restated 2026-08-06, m5-43: instrument narrowed, claim unchanged). **The channel ruling:** the channel an M5 intrusion actually drives is `ZoneDeviceCircuitClosed` → `ZoneStopDemand`, the **latched SF-07-pattern channel** of the cell-scope core — so M5 demonstrates the latched protective stop with monitored-reset recovery. The automatic 2 s inhibit release in the reset row above **remains this function's requirement**; it is a property of the onboard vehicle chain M5 does not build, its verification is **deferred, 2026-08-06 → M6** with that chain (AT-09 requires onboard SF-03 there in any case), and it must never be wired into the latched channel. M5 stand-in for `fieldViolation` = true: the field evaluation's `ZONE` transition and the read-only `Forklift/Safety/ZoneStopDemand` mirror (at M6 the VDA client maps the onboard verdict to `safetyState.fieldViolation`). **M5 form:** (a) spawn an obstacle into the protective field of the moving vehicle → the evaluation reports the violation (`ZONE 0`), `ZoneStopDemand` latches in the consumer's view with **no hand anywhere in the chain**, the mirror rises, the envelope drops and the vehicle reaches standstill **before the field's dimensioned boundary** (Gazebo ground truth). (b) Remove the obstacle → the field reports clear after its 0.3 s debounce and **the latch holds**: no setpoint and no motion returns until the SF-08 monitored reset runs, and motion resumes only on a fresh command after it. (d-1) Repeat the intrusion against the **second** scanner's coverage → the same verdict, the same latch, **no second stop path**. (d-2) Place a target in the **measured** load-direction residual (bearings 164.5–204.4° at 2.0 m, `agv/forklift/EVIDENCE_SENSOR_COVERAGE.md` R3) → **neither** protective field reports it — observed in the run, never inferred from the geometry document; move the same target outside the residual → the field trips and (a) executes, both in one run. **A failing run looks like:** the vehicle still moving after the trip, or coming to rest beyond the dimensioned boundary; the demand or the stop releasing on field-clear alone — auto-release does not belong to this channel; a second stop path acting on the second scanner's trip; or the R3 residual not reproducing where it is measured, which invalidates the coverage evidence rather than the function. **Deferred, 2026-08-06 → M6:** (c) the bumper — the model carries none (`TWIN-DEMO-MAP.md` NC-1 recorded its absence) — lands with the onboard chain; and (d)'s "reduced monitoring case with its ≤ 0.3 m/s SLS limit in force" clause (cross-check AT-10) lands **wherever AT-10 lands** — monitoring-case selection is field-evaluation phase 3 and SLS enforcement is designed nowhere at M5; the AT-10/AT-11 landing ruling is requested in the m5-43 report. Pass: (a), (b), (d-1) and (d-2) observed; (d-2) with its positive counterpart in one run. |
...
| Acceptance test | **AT-04** (Gazebo, M5 — restated 2026-08-06, m5-43). **Instruments:** the warning-field verdict is field-evaluation **phase 2** (`agv/forklift/FIELD-EVALUATION.md` §12) — committed M5 work, not yet built — publishing the creep demand on its named non-safe topic; the M5 observable standing in for VDA `state.velocity` is the vehicle's odometry topic (it becomes `state.velocity` at M6, with the client). The **creep enforcer** — the consumer of the warning verdict — is an open owner/interface decision phase 2 deliberately does not make; until it is decided and built, observation (a)'s actual-speed clause **fails by construction**, stated here rather than weakened: that decision is the one dependency this test carries, requested in the m5-43 report. **M5 form:** (a) obstacle in the warning field only → the creep demand asserts within one field-report cycle, the protective field is untouched, no stop, and commanded **and actual** speed (odometry) fall to ≤ 0.3 m/s; clear the field → the demand withdraws after the 2 s clear-hold and nominal speed resumes. (b) Backup claim: disable the speed-reduction handler (stop the warning-field consumer) and drive at full speed into the same scenario → the SF-03 M5 chain (AT-03 (a)) still stops the vehicle inside the protective field. **A failing run looks like:** actual speed above 0.3 m/s one field-report cycle after the demand asserts; a protective-field trip or any stop on a warning-only intrusion; nominal speed resuming while the field is still occupied; or, in (b), no stop — or a stop produced by the disabled handler rather than by the protective chain. Pass: (a) and (b) observed. |
...
| SF-02 | Vehicle e-stop | VDA `safetyState.eStop` — first producible at M6 with the client; **no M5 stand-in** (AT-02) | AT-02 | M5 (restated stop-and-recovery discipline, 2026-08-06); the onboard e-stop channel, STO semantics and vehicle SF-08 reset instance **deferred → M6** with the onboard chain |
| SF-03 | Protective field stop | VDA `safetyState.fieldViolation` — M5 stand-in: field-evaluation `ZONE` transition + `Forklift/Safety/ZoneStopDemand` mirror | AT-03 | M5 (latched M5 form, 2026-08-06); the 2 s auto-release verification and the bumper case (c) **deferred → M6**; (d)'s SLS-in-force clause lands with AT-10 |
| SF-04 | Warning field speed reduction | none (internal; visible in `state.velocity` — M5 stand-in: the odometry topic) | AT-04 | M5 (dependent on the creep-enforcer decision, see AT-04) |

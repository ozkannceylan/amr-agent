...
acceptance test. F-CPU functions are verified at gate M7; vehicle-side
behaviors are demonstrated in simulation at M3/M4.
...
### 1.3 Arm safety — out of scope until M9

The RB-KAIROS platform is a mobile manipulator (ADR 0002); the arm exists in
the model from M3 but arm safety functions are **not specified here**. The ID
block **SF-20 … SF-29** is reserved for arm safety at gate M9 (expected
contents per ADR 0002: base-stationary interlock as precondition for arm
motion, separate base/arm safety zones). Until M9, the arm is parked and
unpowered in every scenario; no SF below claims to cover arm motion.
...
| Acceptance test | **AT-08** (PLCSIM Advanced, M7): After tripping SF-01: (a) hold reset input high permanently → latch stays, reset-fault flagged. (b) Pulse 0→1→0 within 100 ms (< 0.2 s) → rejected. (c) Valid pulse (1 s) with the e-stop still tripped → rejected. (d) Clear e-stop, valid pulse → latch releases exactly on the falling edge, `SafetyResetRequired` 1→0, and **no output energizes** as a consequence of the reset alone. Pass: all four. |
...
| Acceptance test | **AT-09** (Gazebo + broker, M4): Kill the MQTT broker while the vehicle executes an order → vehicle decelerates to a controlled stop within the watchdog period, keeps the order, `connection` last-will fires. During the outage, trip the simulated protective field → SF-03 still acts (independence from network, B1/B2). Restore the broker → vehicle resumes after fleet resync, without operator reset. Pass: all three. |
...
|---|---|---|---|---|
| SF-01 | Cell e-stop chain | OPC `Safety/EStopActive` | AT-01 | M7 |
| SF-02 | Vehicle e-stop | VDA `safetyState.eStop` | AT-02 | M3 (sim behavior), M7 review |
| SF-03 | Protective field stop | VDA `safetyState.fieldViolation` | AT-03 | M3 |
| SF-04 | Warning field speed reduction | none (internal; visible in `state.velocity`) | AT-04 | M3 |
| SF-05 | Door interlock | OPC `Safety/SafetyDoorClosed`, `ProtectiveStopActive` | AT-05 | M7 |
| SF-06 | Charger interlock | OPC `Charger/ChargerContactorClosed` (process mirror) | AT-06 | M7 |
| SF-07 | Zone monitoring | OPC `Safety/ProtectiveStopActive`, `Cell/ZoneAOccupied` (process) | AT-07 | M7 (end-to-end at M8) |
| SF-08 | Monitored reset | OPC `Safety/SafetyResetRequired` | AT-08 | M7 |

...
| Clamping `ForkliftSteerAngleRef` to the mechanical range, or centring it when the machine stops | Both are process decisions the PLC makes and states: the clamp is the PLC's, and steering is deliberately **not** gated to zero, because a steer setpoint is a position and forcing it to centre would move the wheel of a machine that is supposed to be stopping (§10.6) | PLC |
...
| **Restart residual** | A revert that lands on exactly the value this session last wrote is invisible to the test — one heartbeat value in 65536, and the next restart is still caught. A restart that *does* break the session is caught by the *Detection* row instead, which invalidates the same cache. Stated rather than patched: patching it needs a second witness, and a second witness needs an owner |
...
| 8 | m3-01 open question 6 (stale "Navigation scenario (M3)" heading in `sim/README.md`) | **Corrected in `sim/`**: the heading now reads "Navigation scenario (M5, deferred)". **Re-opened by ADR 0008 D1**, which shifted every gate above M3 by one, so that heading now names the wrong gate — the vehicle gate is M6. `sim/`'s to correct; requested, not edited here (item 15) |
...
| 11 | Implementing the configured-signal-set model in `bridge/` | **Open, `bridge/`'s** (m4f-06). What this document now requires and the shipped code does not yet do: the write allowlist **derived** from the configured groups rather than from a fixed key list (§4.10); R3's count taken from the configured set rather than from the seven cell inputs (§6.1); the same for the reconnect refresh and the restart repair (§8.1); and the startup/withheld log lines worded per configured set rather than naming `DemoCell/Input` (§9.3). The design is the contract; the code follows it, not the reverse |
...
| 13 | `bridge/EVIDENCE_LATENCY.md`'s standing request: "§8.1's *Detection* row … the design document does not carry it. It needs a row, and the bridge's own log cites §8.1 for a rule that is not yet there" | **Resolved here**: §8.1 gains *Restart detection*, *Restart repair* and *Restart residual*, §7.3 gains case E, §2's cycle description gains step 0, §4.3 gains row 9r, and §9.2 gains RB. The log line's citation now resolves to a rule that exists. The requesting file is in `bridge/` and cannot be edited from here — **`bridge/` marks its own request satisfied** (m4f-06) |
| 14 | Statements elsewhere that were true of a one-group interface and are now scope-stale | **Open, requested, not edited here.** `plc/demo-cell/SPEC.md` §4.3 "Nothing else goes into the interface" (true of the M3 cell, no longer true of the interface — `plc/`'s); and `bridge/README.md` / `bridge/EVIDENCE_LIFECYCLE.md` "the only node outside `Input/`", which stays defensible under the revised `opcua-nodes.md` §9.7 wording but should be confirmed against §4.10's write set (`bridge/`'s) |
| 15 | `sim/README.md` § "Navigation scenario (M5, deferred)" | **Open, `sim/`'s.** ADR 0008 D1 shifted every gate above M3 by one, so the vehicle gate is M6 and this heading names the wrong one. Raised from item 8, which this document owns the history of but not the file |

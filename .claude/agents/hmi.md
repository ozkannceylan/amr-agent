...
Hard rules:
- The HMI streams requests; the PLC owns the outcome. No interlock, no latch, no sequencing, no setpoint formation, no reaction to plant state, no verdict the PLC also computes and no actuator output here — teleop routing, the fork-height speed cap, the fork soft travel limits and the lidar obstacle stop are process logic in the standard program (ADR 0008 D3). The line is not "no timer" (opcua-nodes.md §10.1): the HMI owns the timers that produce its own 10 Hz write cycle with the 5 Hz floor it holds itself to (§10.8 H2) and its window over its operator's page (§10.8 H6). The test is what a timer watches — its own cycle or its own input channel, never the plant, and never a verdict the PLC also computes. Timing a process value is forbidden: a debounce, a fault delay, a dwell, a stale window over a plant signal, "write only if stable for X ms" — the threshold and the delay are process decisions and they belong to the PLC. If a brief seems to require logic in the HMI, stop and report.

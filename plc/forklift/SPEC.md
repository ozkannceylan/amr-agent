...
| 5.4.4 | **Attempt a reset while the obstacle is still in the zone**: assert and release the reset control | The reset is **refused**. `ForkliftResetRequired` stays `TRUE`, `ForkliftObstacleStopActive` stays `TRUE`, nothing moves. `CauseGone` is false on C3 |
| 5.4.5 | Attempt to drive out of the zone: release and re-assert the enable | **Refused** — teleop cannot be enabled while a latch stands, so the machine cannot drive itself clear. **This is a deliberate consequence, not a defect** (§12): the zone is cleared by moving the obstacle, and no creep-out mode exists |
| 5.4.6 | Clear the zone by removing the obstacle in Gazebo | `ForkliftObstacleInStopZone` → `FALSE`; `ForkliftObstacleStopActive` stays **`TRUE`**. **The field clearing does not release the latch** |
| 5.4.7 | **Stuck reset**: assert the reset control and leave it asserted | The latch **never** clears, for as long as it is held. There is no edge to act on, and no elapsed time makes one appear |
| 5.4.8 | Release the reset control, then assert it again | All five latches clear on that rising edge; `ForkliftResetRequired` → `FALSE`; **nothing moves** and `ForkliftTeleopActive` stays `FALSE` |
| 5.4.9 | Assert the enable (a fresh edge) | Teleop returns and the refs follow the operator's controls again. **Reset and enable are two separate, deliberate actions**, and this step is where the §6.7 conflation is demonstrated |

...
| K2 | §6.5 | The cap engages at `FORK_HEIGHT_SLOW_THRESHOLD` **with the operator's control untouched**: 1.00 → 0.30 m/s on the fork crossing 0.50 m. At a 0.2 demand the setpoint is 0.06, so the cap limits rather than commands, and `ForkliftSpeedLimitActive` stays `TRUE` — "in force", not "biting" |

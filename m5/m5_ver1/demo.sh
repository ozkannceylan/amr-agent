...
#
# ===========================================================================
# VIRTUAL-PLC ERA (2026-08-21). This is m5/m5_ver1/demo.sh, a copy of the
# recorded demonstration's .archive/demo.sh with exactly five marked changes,
# each prefixed "VIRTUAL-PLC ERA:". The PLCSIM Advanced trial has expired, so
# the Windows half is now m5/m5_ver1/virtual_plc/virtual_plc.py -- a software
# stand-in that serves the same 43-node address space, runs behavioural models
# of both PLC programs, and plays the stand-in writer's role (mutex, the two
# TCP links, the operator commands). IT IS NOT A PLC and carries no safety
# integrity; neither did the writer it replaces. The five changes:
#
#   1. REPO resolves two levels up (this file lives in m5/m5_ver1/).
#   2. The Windows pre-flight accepts the virtual PLC where it accepted the
#      PLCSIM runtime process: the OPC UA listener on 4841 is probed the same
#      way. The CPU-in-RUN proof is unchanged: the bridge's session.
#   3. The writer command this script prints is the virtual PLC's.
#   4. teardown's writer sweep also matches virtual_plc.py command lines.
#   5. Unless AMR_BRIDGE_CONFIG / AMR_HMI_CONFIG are set, the bridge and HMI
#      configs are GENERATED into the runtime dir from the committed ones,
#      with the endpoint line substituted for the derived Windows host
#      address and the virtual PLC's port. No committed file is edited.
#
# Everything below the marked lines is the archived script, verbatim.
# ===========================================================================
#
# ===========================================================================
...
# VIRTUAL-PLC ERA 1: this copy lives in m5/m5_ver1/, so the repo root is two
# levels up. The archived script resolved its own directory.
...
windows_state() {
    ...
    "$POWERSHELL" -NoProfile -NonInteractive -Command '
        ...
        $v = @(netstat -ano | Select-String -Pattern "LISTENING" | Select-String -Pattern ":4841\s")
        Write-Output ("virt=" + ($v.Count -gt 0))
    ' 2>/dev/null | tr -d '\r'
}
...
print_writer_command() {
    # VIRTUAL-PLC ERA 3: the command printed is the virtual PLC's. It holds
    # the same mutex, listens on the same two ports and takes the same
    # operator words (estop/zone/reset pulse) on its console and command file.
    note ""
    note "  Open a Windows terminal ON THE WINDOWS SIDE, cd to the"
    note "  repository, and run this. Leave the window open - it is the"
    note "  operator keyboard for estop, zone and reset:"
    note ""
    note "    python m5\\m5_ver1\\virtual_plc\\virtual_plc.py --command-file C:\\Temp\\m5v1_cmds"
    note ""
    note "  (The recorded path ran $WRITER_SCRIPT_WIN against PLCSIM"
    note "  Advanced. The trial has expired; the virtual PLC stands where it stood.)"
    note ""
}
...
cmd_check() {
    ...
    v="$(windows_field "$state" virt)"

    # VIRTUAL-PLC ERA 2: the CPU seat is filled by PLCSIM Advanced's runtime
    # process OR by the virtual PLC's OPC UA listener on 4841. Either answer
    # is printed as what it is; the CPU-in-RUN proof is unchanged and is still
    # the bridge's session, not a process and not a port.
    if [ "$p" = "True" ]; then
        ...
        fail "no PLCSIM Advanced runtime instance process, and no virtual PLC"
        fail "listener on 4841. The trial has expired, so the CPU seat is filled"
        fail "by:  python m5\\m5_ver1\\virtual_plc\\virtual_plc.py --command-file C:\\Temp\\m5v1_cmds"
        fail "run on the Windows side. That is yours to start; this script never"
        fail "starts it."
        problems=$((problems + 1))
    fi
...
cmd_up() {
    ...
    # VIRTUAL-PLC ERA 5: unless the caller named configs, generate the bridge
    # and HMI configs into the runtime dir from the committed ones, with the
    # one endpoint line substituted for the Windows host address THIS WSL
    # routes to (read back from the default route, never assumed) and the
    # virtual PLC's port. No committed file is edited; git status stays clean.
    if [ -z "${AMR_BRIDGE_CONFIG:-}" ] || [ -z "${AMR_HMI_CONFIG:-}" ]; then
        local win_host
        ...
        if [ -z "${AMR_BRIDGE_CONFIG:-}" ]; then
            BRIDGE_CONFIG="$RUN_DIR/bridge.virtual.yaml"
            sed "s|^  endpoint: \"opc.tcp://[^\"]*\"|  endpoint: \"opc.tcp://$win_host:4841\"|" \
                "$REPO/bridge/config/bridge.yaml" > "$BRIDGE_CONFIG" \
                || die "could not generate $BRIDGE_CONFIG"
            info "generated $BRIDGE_CONFIG (endpoint opc.tcp://$win_host:4841)"
        fi
        if [ -z "${AMR_HMI_CONFIG:-}" ]; then
            HMI_CONFIG="$RUN_DIR/hmi.virtual.yaml"
            sed "s|^  endpoint: \"opc.tcp://[^\"]*\"|  endpoint: \"opc.tcp://$win_host:4841\"|" \
                "$REPO/hmi/config.yaml" > "$HMI_CONFIG" \
                || die "could not generate $HMI_CONFIG"
            info "generated $HMI_CONFIG (endpoint opc.tcp://$win_host:4841)"
        fi
    fi
...
stop_writer() {
    ...
    # VIRTUAL-PLC ERA 4: the sweep matches the virtual PLC's command line as
    # well as the recorded writer's. Same identity rule: by command line,
    # never by name, never by port, and never this probe itself.
        ...
        function Find-Writer {
            @(Get-CimInstance Win32_Process |
              Where-Object { $_.ProcessId -ne $PID -and $_.CommandLine -and
                             ($_.CommandLine -match "standin_writer\.ps1" -or
                              $_.CommandLine -match "virtual_plc\.py") })
        }
        $procs = Find-Writer

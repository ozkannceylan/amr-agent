...
#   a command line BYTE-IDENTICAL to m6_world.launch.py's - same absolute
...
#   owner started with GZ_PARTITION=m6 is by that act IN this graph.
...
#   that would be is a second m6 stack's broker.
recorded() {
...
sweep() {  # sweep <signal>
    ...
    for pat in "${PATTERNS[@]}"; do
        while read -r pid cmd; do
            ...
            case "$cmd" in *m6.sh*|*demo.sh*|*stack.sh*) continue ;; esac
            ours "$pid" || continue
...
deploy() {
    ...
    # deploy/m6/ipc/cmd_gate.py finds its config at
    # deploy/m6/vehicles/<vid>/config.yaml through the same
    ...
    ( cd "$M6" && python3 tools/instantiate_vehicle.py --all ) || return 1
    rm -rf "$DEPLOY"
    ...
    ( cd "$DEPLOY" && {
        echo "# m6 deploy - generated, do not edit"
...
start() {
    ...
    # broker that is not there. Same realistic holder as above: a m6
    ...
    if [ ! -x "$BROKER_BIN" ]; then
        ...
        echo "run 'bash $M6/tools/install_broker.sh' first - it needs no sudo."
        return 1
    fi

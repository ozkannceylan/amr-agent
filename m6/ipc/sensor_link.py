...
sensor port (VEHICLES table), where m6.py writes PF_OSSD, WF_Clear
...
# leaving and arrives at m6.py looking FRESH. At rest that pair is
# 0/0, which m6.py and the design both name the most dangerous lie
...
def payload(fields_json, encoders_json):
    ...
    ONE DATAGRAM CARRIES EVERYTHING so m6.py has one staleness rule
...
class SensorLink(Node):
    ...
    def cb_fields(self, msg):
        ...
        # m6.py's own timeout then writes the trip values for both.

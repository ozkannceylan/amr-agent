"""m6.py's encoder fault injection."""
import m6


def test_ok_passes_both_channels_through():
    assert m6.apply_encoder_fault(400, 402, "ok", 0) == (400, 402)
...
def test_fa_freezes_channel_a_at_its_last_value():
    ...
    assert m6.apply_encoder_fault(400, 402, "fa", 0) == (0, 402)
    assert m6.apply_encoder_fault(900, 902, "fa", 400) == (400, 902)


def test_oa_offsets_channel_a_past_the_cross_check():
    a, b = m6.apply_encoder_fault(100, 102, "oa", 0)
    assert a == 100 + m6.ENC_OFFSET_MM_S
    assert abs(a - b) > 50      # the F-program's limit


def test_an_unknown_mode_passes_through_rather_than_inventing_a_fault():
    assert m6.apply_encoder_fault(400, 402, "wat", 0) == (400, 402)
...
def test_the_stale_values_fail_both_f_program_checks():
    ...
    assert abs(m6.ENC_STALE_A - m6.ENC_STALE_B) > 50
    assert max(m6.ENC_STALE_A, m6.ENC_STALE_B) > 2800
...
def test_parse_sensor_requires_integer_encoder_fields():
    ...
    def packet(**enc):
        msg = {key: True for key, _tag in m6.SENSOR_TAGS}
        msg.update({"enc_a": 10, "enc_b": 12, "ts": 1.0})
        msg.update(enc)
        return json.dumps(msg).encode()

    assert m6.parse_sensor(packet())["enc_a"] == 10
    assert m6.parse_sensor(packet(enc_a="10")) is None
    assert m6.parse_sensor(packet(enc_a=True)) is None
    bad = json.loads(packet().decode())
    del bad["enc_a"]
    assert m6.parse_sensor(json.dumps(bad).encode()) is None

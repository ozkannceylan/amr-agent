...
HEALTHY = {"estop_healthy": True, "motor": True, "case": 3, "v_limit": 1500, "ts": 0.0}
...
def test_the_enable_line_is_separate_from_the_lamp():
    ...
    # With a permission given, it rides beside the enable rather than
    # taking a sixth lamp: both answer "what may the truck do now".
    assert hmi_node.enable_text(True, 1500) == "Drive enable: ON  ·  limit 1.50 m/s"
    assert hmi_node.enable_text(True, 300) == (
        "Drive enable: ON  \u00b7  limit 0.30 m/s")
...
def test_a_fresh_status_is_shown_as_it_arrived():
    assert hmi_node.display_state(HEALTHY, 0.0, 0.10) == (
        hmi_node.LAMP_NEUTRAL, "E-Stop Inactive", "Drive enable: ON  ·  limit 1.50 m/s")
...
def test_the_latch_display_survives_the_staleness_rule():
    ...
    colour, lamp, enable = hmi_node.display_state(
        {"estop_healthy": True, "motor": False, "case": 3, "v_limit": 1500, "ts": 0.0}, 0.0, 0.10)
    assert (colour, lamp) == (hmi_node.LAMP_NEUTRAL, "E-Stop Inactive")
    assert enable.startswith("Drive enable: OFF")
...
def test_a_status_that_stopped_arriving_shows_the_safe_display():
    ...
    assert hmi_node.display_state(HEALTHY, 0.0, 0.30) == (
        hmi_node.LAMP_RED, "E-Stop Active", "Drive enable: OFF  ·  limit 0.30 m/s")
...
class _StubLabel:
    """Stands in for a tk.Label: only cget and configure are used."""
...
class _StubHmi:
    ...
    def __init__(self):
        ...
        self.encoder_widget = _StubLabel()
        self.sensor_lamps = {}
        self.shown = None


LIVE = '{"estop_healthy": true, "motor": true, "case": 3, "v_limit": 1500, "ts": 0.0}'
...
def test_a_tick_shows_the_status_that_arrived(monkeypatch):
    ...
    assert node.enable.told["text"].startswith("Drive enable: ON")
...
def test_a_later_tick_goes_safe_with_no_further_status(monkeypatch):
    ...
    assert node.enable.told["text"].startswith("Drive enable: OFF")

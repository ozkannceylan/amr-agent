...
THE FLOOR IS SOMEBODY ELSE'S ROOM (floor.py). Every reservation, hold,
base extension, deadlock and idle sweep lives there, in a `Floor` that
owns traffic.py's ledger and each task's traffic record; this file
reaches all of it through `self.floor` and through nothing else. The one
sentence worth repeating on this side is the rule that licenses any of
it: RESERVATION IS PROCESS DECONFLICTION AND NEVER A COLLISION CLAIM -
the scanners, the F-model and the onboard guards are the only things
that stop a truck, and what the ledger buys is that the fleet never ASKS
two vehicles onto one piece of floor. `--no-traffic` turns the whole
room off and grants every route whole, which is the M6.3 behaviour and
is how the gates reproduce the jam on purpose.
...
class FleetManager:
    ...
    def __init__(self, host=MQTT_HOST, port=MQTT_PORT, traffic_on=True):
        ...
        # The floor is a room of its own now (floor.py) and this is the
        # only handle on it: the ledger, the per-task traffic records,
        # the holds and everything --no-traffic turns off.
        self.floor = Floor(self, traffic_on)
        self.stop = False
    ...
    def drain(self):
        ...
        # The floor's pass first: every leg already on the floor gets its
    ...
    def _check_rejection(self, serial, veh, msg):
        ...
        for err in errors if isinstance(errors, list) else []:
            ...
            if not any(isinstance(r, dict)
                ...
                continue
            if self.floor.extension_refused(serial, task, err):
                return
            veh["not_eligible"] = True
    ...
    def _assign(self, now):
        ...
        order, trf = self.floor.leg_order(serial, task, order_id,
                                          view[serial]["position"],
                                          task["from"], "leg1")
        if order is None:
            return
        if not self._publish_order(serial, order, task, "leg1"):
            self.floor.release(serial)
            return
        task["traffic"] = trf
    ...
    def _publish_extension(self, serial, task, order, released, update):
        ...
        self._note_history(task, "base {} released + {} horizon (upd {})"
                           .format(released, horizon, update))
        return True
    ...
    def _note_history(self, task, line):
        """A row on the operator's screen, bounded. The history is not an
        audit trail: a task that loops between trucks must not grow the
        retained document without bound. The floor writes through here
        rather than trimming the manager's book itself."""
        task["history"].append(line)
        del task["history"][:-HISTORY_MAX]

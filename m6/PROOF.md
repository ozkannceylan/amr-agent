...
M6.3, the fleet gates (numbered FLEET 1..6), all measured 2026-08-22
00:16-01:14 by the scripted driver + the fleet CLI:
...
[ ] FLEET 4 - vehicle loss mid-task           MEASURED AND FAILED
              (the fleet half is clean - CONNECTIONBROKEN in 92 ms,
               requeue-to-head and reassignment in 1 ms. The vehicle
               half is not: the cancelOrder was FINISHED in 132 ms and
               did not stop the truck, which drove 37.09 s / 6.743 m
               driverless, and the replacement could not pass it.
               Two defects, written up in the gate.)
...
Five of the six gates passed on the measurement. **Fleet Gate 4 failed,
and it failed on two real defects** — both are written up with their
numbers below and neither is hidden behind a tick.
...
## [ ] Fleet Gate 4 — vehicle loss mid-task — MEASURED AND FAILED

**Verdict: FAIL.** *Kill one agent mid-leg → CONNECTIONBROKEN → task to
queue head → other vehicle completes it; the lost vehicle returns, gets
cancelOrder for its stale order, re-earns eligibility.*

**The fleet half of this gate is clean. The vehicle half is not, and it
took two defects with it: the cancelOrder did not stop the truck, and
the replacement could not get past it.**
...
**The fleet reacted in 93 ms and reassigned in one more:**
...
pid appended to `.m6_pids`, `env VEHICLE=f1`, running from `deploy/`,
under `GZ_PARTITION=m6` so `stop` still sweeps it:

```
01:06:05.186  respawned vda_agent_f1 as pid 62225
$ tr '\0' ' ' < /proc/62225/cmdline
python3 .../m6/deploy/m6/ipc/vda_agent.py
$ tr '\0' '\n' < /proc/62225/environ | grep -E 'VEHICLE|GZ_PARTITION'
...
### Defect 1 — the cancelOrder reached the agent and did not stop the truck
...
**132 ms, FINISHED.** `_on_actions` publishes the empty goal on
`/f1/auto/goal` unconditionally on that path — and **the topic gained no
sample.** `/f1/auto/goal` carries exactly two samples in the whole
session, 01:04:54.934 and 01:08:49.471, both scratch cancels sent to a
...
and it arrives into an unmatched publisher. The fleet cannot stop a
truck whose agent it just restarted.

### Defect 2 — the replacement could not pass the lost vehicle

f2 took the task at 01:06:03.064 and drove west. It never reached S3:

```
f2 stopped 01:06:42.062 at (-4.6520, -5.5036)
f1 parked  01:06:40.065 at (-6.8493, -6.0421)
centre-to-centre 2.262 m
f2 nav: SAFETY-STOP   both trucks motor-false from 01:06:41.9
```

A vehicle standing anywhere on the dock aisle or on a spur shorter than
about 3.5 m is an obstacle its replacement cannot get past: the
protective field stops it and the safe-speed monitor latches both. This
is **exactly the non-goal M6.3 declared** ("No traffic logic… the
onboard guards and the safety chain are what prevents contact") and it
is the concrete argument for M6.4: on this floor, requeue-to-head is
only useful if the replacement can physically reach the work.

### What passed inside the failure

```
kill -> CONNECTIONBROKEN                            92 ms
CONNECTIONBROKEN -> requeue to head + reassign       1 ms
agent respawn -> ONLINE seen by the manager       1.086 s
manager -> cancelOrder on the wire                   1 ms
cancelOrder -> actionStatus FINISHED               132 ms
```

Also measured, and a smaller defect of its own: **the restarted agent
came back MANUAL.** `/fN/hmi/mode` is TRANSIENT_LOCAL and there are two
latched publishers on it (the recorder's `auto` and `hmi_node`'s own);
the new agent took one, then the other. The manager's
`f1 re-earned eligibility` at 01:06:06,674 fired on a single AUTOMATIC
state at 01:06:06.580 that had flipped back to MANUAL by 01:06:07.696.
Re-earning eligibility off one sample is correct by the written rule and
still wrong here.

**No tick. The gate is measured and failed, and the two defects above
are the deliverable.**
...
1. **A restarted agent's cancelOrder cannot stop its truck** (Fleet
   Gate 4). The action is received and FINISHED in 132 ms; the empty
   goal it publishes reaches nobody because the rclpy publisher is
   ~1 s old and `nav_node` has not matched it. Measured cost: 37.09 s
   and 6.743 m of a driverless truck completing a leg that belonged to
   another vehicle. The vehicle-side fix is to wait for
   `get_subscription_count() > 0` (or re-publish) before trusting a
   goal published in a node's first seconds; the fleet-side mitigation
   is to keep `self.stale[serial]` until the vehicle's own state proves
   the order is gone, instead of deleting it on a successful publish.
